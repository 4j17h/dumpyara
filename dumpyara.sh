...
[[ -z "${brand}" ]] && brand=$(grep -aoP "(?<=^ro.product.product.brand=).*" -hs vendor/euclid/product.img | head -1)
...
[[ -z "${codename}" ]] && codename=$(grep -aoP "(?<=^ro.product.product.model=).*" -hs vendor/euclid/product.img | head -1)
